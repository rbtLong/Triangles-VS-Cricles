...
{
    public class Type1Variant3 : BaseEnemy
    {
        public Type1Variant3(string question, string answer,
            IAttackable target, IMoveStrategy moveStrategy)
            : base(question, answer, target, moveStrategy)
        {
        }

                public Type1Variant3(string life,
            IAttackable target, IMoveStrategy moveStrategy)
            : base(life, target, moveStrategy)
        {
        }

        public Type1Variant3(IEnumerable<string> life, 
            IAttackable target, IMoveStrategy moveStrategy) 
            : base(life,target, moveStrategy) { }
        public Type1Variant3(char life, IAttackable target,
            IMoveStrategy moveStrategy)
            : base(life, target, moveStrategy) { }

        protected override BaseAttack InitializeAttack()
        {
            return new LinearScatter(this, Target, 5, 8)
            {
                Frequency = TimeSpan.FromMilliseconds(1200),
                AttackSpeed = TimeSpan.FromMilliseconds(21000)
